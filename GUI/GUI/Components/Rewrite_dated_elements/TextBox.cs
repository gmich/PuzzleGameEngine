...
namespace GUI.Components
{
    ...
    public class TextBox
    {
        ...
        //include windows.forms dll
        #region ClipBoard

        public void GetInfoFromClipboard()
        {
            try
            {
                ...
                {
                    string clipboardText = (string)d.GetData(DataFormats.Text);
                    keyboardManager.Text += clipboardText;

                    if (keyboardManager.Text.Length > 17)
                    {
                        keyboardManager.Text=keyboardManager.Text.Remove(17);
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            if ((InputManager.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.LeftControl)
                 && InputManager.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.V)) && !checkClipboard)
            {
                GetInfoFromClipboard();
                checkClipboard = true;
            }
            if (!(InputManager.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.LeftControl)
                && InputManager.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.V)))
                checkClipboard = false;
        }
